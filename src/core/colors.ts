...
/**
 * 미리보기의 도안 선. 재단선·완성선·중앙선이 이 색 하나에 같은 두께로 나간다.
 *
 * 인쇄는 셋을 색으로 가른다(CUT_COLOR·SEAM_COLOR·CENTER_COLOR). 화면만
 * 합치는 것이라 이 파일에서 화면과 인쇄가 일부러 갈라지는 유일한 자리다.
 * colors.ts를 만든 까닭이 "같은 선이 매체마다 다른 색이 되는 사고"를 막으려던
 * 것이므로, 여기가 사고가 아니라 결정이라는 걸 적어 둔다.
 *
 * 화면에서 색을 안 나눠도 되는 근거는 다른 단서가 이미 있기 때문이다.
 * 재단선과 완성선 사이는 시접 띠(SEAM_BAND_FILL)가 분홍으로 차 있어 바깥이
 * 재단선, 안쪽이 완성선임이 드러나고, 중앙선은 일점쇄선이라 실선과 섞이지
 * 않는다. 종이는 사정이 다르다 — 띠를 칠하지 않으므로 색이 유일한 단서다.
 *
 * 사각과 원통 두 미리보기가 이 상수를 함께 쓴다. 원통에는 원래 시접 띠가
 * 없었는데, 선을 합치면서 함께 넣었다 — 띠가 없으면 색도 두께도 같은 두
 * 선만 남아 어느 쪽을 잘라야 하는지 알 수 없다. 띠가 이 결정의 전제다.
 */
export const PREVIEW_LINE_COLOR = SEAM_COLOR;
...
/**
 * 식서방향. 원단 결을 어느 쪽으로 놓고 재단할지 알려 주는 양끝 화살표다.
 *
 * 조립 표시(MARK_COLOR)에 얹지 않는다. 그쪽은 종이를 이어붙이는 표시이고
 * 식서는 원단에 대한 지시라, 값이 비슷하다고 묶으면 개념이 뭉개진다.
 *
 * 녹색인 까닭은 팔레트에 녹색이 없어서다. 빨강 셋(골선·축척·맞춤표)은
 * "조심하라"는 뜻으로 이미 차 있고 청록은 페이지 경계가 쓴다. 결 방향은
 * 경고도 페이지도 아니므로 둘 다 비켜서야 한다.
 *
 * 미리보기 배경(--tint-pattern) 위에서 5.62:1 — 선 기준 3:1을 넘는다.
 */
export const GRAIN_COLOR = '#2f6f3e';
