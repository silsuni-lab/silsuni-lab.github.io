...
export const KOREAN_FONT_CHARS: ReadonlySet<string> = new Set(
  " !*@_0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabchilmnsu각게골들만보사선세시쁘없어예요우음인접지치퍼파하확·글껑닥단동뒷뚜랫면바아앞원윗장통네모손잡이",
);
...
/**
 * 식서선 굵기 (pt). 처음엔 도안 선보다 가는 0.35였는데, 인쇄하면 흐려서 0.5로 올렸다.
 * 이 굵기는 맞춤표 같은 다른 선도 쓰므로 굵기로 식서선을 가려낼 수 없다 —
 * 테스트는 식서선 좌표에 그어진 선을 찾는다.
 */
export const GRAIN_THICKNESS = 0.5;
