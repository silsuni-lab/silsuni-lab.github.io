...
describe('renderShapeSvg', () => {
  it('svg 요소를 만든다', () => {
    expect(renderShapeSvg(cosmetic, 'ko')).toMatch(/^<svg /);
    expect(renderShapeSvg(cosmetic, 'ko')).toMatch(/<\/svg>$/);
  });

  it('앞면이 가로·높이 실제 비율을 그대로 쓴다', () => {
    const face = frontFace(renderShapeSvg(cosmetic, 'ko'));
    expect(face.widthMm / face.heightMm).toBeCloseTo(150 / 90, 6);
  });

  it('납작한 파우치와 도톰한 파우치의 앞면 비율이 다르다', () => {
    const flat = frontFace(renderShapeSvg(pencil, 'ko'));
    const tall = frontFace(renderShapeSvg(cosmetic, 'ko'));
    expect(flat.widthMm / flat.heightMm).toBeGreaterThan(tall.widthMm / tall.heightMm);
  });

  it('바닥폭이 클수록 깊이 방향으로 더 멀리 물러난다', () => {
    const shallow = renderShapeSvg({ widthMm: 150, heightMm: 90, depthMm: 50 }, 'ko');
    const deep = renderShapeSvg({ widthMm: 150, heightMm: 90, depthMm: 200 }, 'ko');
  ...
  it('앞면·윗면·옆면 세 면을 그린다', () => {
    const svg = renderShapeSvg(cosmetic, 'ko');
  ...
  it('숨은 모서리를 점선으로 그린다', () => {
    expect(renderShapeSvg(cosmetic, 'ko')).toMatch(/class="hidden-edge"[^>]*stroke-dasharray/);
  });

  it('지퍼단을 표시한다', () => {
    expect(renderShapeSvg(cosmetic, 'ko')).toContain('class="zipper"');
  });
  ...
  it('소수점 치수를 첫째 자리까지만 적는다', () => {
    const svg = renderShapeSvg({ widthMm: 150, heightMm: 90, depthMm: 95 }, 'ko');
    expect(svg).toContain('95mm');
    expect(svg).not.toMatch(/\d\.\d\d/);
  });

  it('그림 설명을 aria-label로 제공한다', () => {
    expect(renderShapeSvg(cosmetic, 'ko')).toMatch(/aria-label="[^"]+"/);
  });
});
...
describe('renderShapeSvg — 크기에 따른 일관성', () => {
  ...
  it('작은 파우치와 큰 파우치의 글자 크기가 그림에 비례한다', () => {
    const small = renderShapeSvg({ widthMm: 100, heightMm: 50, depthMm: 40 }, 'ko');
    const large = renderShapeSvg({ widthMm: 400, heightMm: 300, depthMm: 200 }, 'ko');
...
describe('renderShapeSvg — 라벨이 그림 밖으로 넘치지 않는다', () => {
  ...
  it('바닥폭 라벨이 오른쪽 끝을 넘지 않는다', () => {
    for (const dims of [
      ...
      const svg = renderShapeSvg(dims, 'ko');
  ...
  it('모든 라벨이 viewBox 안에 있다', () => {
    const svg = renderShapeSvg({ widthMm: 150, heightMm: 90, depthMm: 50 }, 'ko');
...
describe('renderShapeSvg — WebKit 크기 계산', () => {
  ...
  it('폭을 명시해 WebKit이 0으로 계산하지 않게 한다', () => {
    const svg = renderShapeSvg(pencil, 'ko');
    const style = svg.match(/<svg[^>]*style="([^"]*)"/)![1]!;
    expect(style).toMatch(/(^|;)\s*width:\s*100%/);
  });

  it('높이는 비율대로 따라오게 둔다', () => {
    const style = renderShapeSvg(pencil, 'ko').match(/<svg[^>]*style="([^"]*)"/)![1]!;
...
describe('renderShapeSvg — 지퍼가 옆면으로 이어진다', () => {
  ...
  it('옆면 지퍼가 세로로 높이의 절반만큼 내려온다', () => {
    for (const dims of [pencil, cosmetic]) {
      for (const line of lines(renderShapeSvg(dims, 'ko'), 'zipper-side')) {
