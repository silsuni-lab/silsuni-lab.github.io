...
/*
 * 그리는 파일 전부. 원통이 늘면서 여섯이 됐다. 새 파일을 만들 때 여기에
 * 더하지 않으면 그 파일만 규칙 밖에 남는다.
 */
const DRAWING_FILES = [
  'src/ui/preview.ts',
  'src/ui/shape.ts',
  'src/ui/round/preview.ts',
  'src/ui/round/shape.ts',
  'src/core/pdf.ts',
  'src/core/page.ts',
  'src/core/round/pdf.ts',
];

describe('색은 colors.ts에서만 짓는다', () => {
  for (const file of DRAWING_FILES) {
  ...
  /*
   * rgb()를 부르는 자리는 page.ts의 pdfColor 하나뿐이다. 사각·원통 PDF가
   * 그 함수를 가져다 쓴다. 두 곳에서 색을 지으면 같은 선이 매체에 따라
   * 달라지고, 그건 사람 눈으로 못 잡는다 — 지금까지 두 번 그렇게 됐다.
   */
  it('rgb()는 page.ts의 pdfColor 안에서 한 번만 부른다', () => {
    const calls = stripBlockComments(read('src/core/page.ts')).match(/\brgb\(/g) ?? [];
    expect(calls).toHaveLength(1);
  });

  for (const file of ['src/core/pdf.ts', 'src/core/round/pdf.ts']) {
    it(`${file}는 rgb()를 직접 부르지 않는다`, () => {
      const calls = stripBlockComments(read(file)).match(/\brgb\(/g) ?? [];
