...
어떤 사이즈를 몇 명이 뽑아 가는지 남긴다. 프리셋과 기본값을 손볼 때 짐작
대신 근거를 쓰려는 것이다. 사각과 원통 두 화면이 같은 시트로 보낸다.
...
| `언어` | `ko`/`en`/`zh-TW`/`zh-CN`/`ja` | 어느 언어 페이지에서 받았는지 |
...
열 이름이 원통에는 안 맞지만 `종류`를 보면 무슨 값인지 알 수 있다. 집계할
때 `종류`로 먼저 거르지 않으면 뜻이 다른 값을 한데 더하게 된다.
...
```
시각	탭	종류	가로	높이	바닥폭	용지	시접	반접기	언어
```

**이미 쓰던 시트라면** `탭` 다음에 열 하나를 끼워 넣고 머리글을 `종류`로
적은 뒤, 이미 쌓인 줄의 그 칸을 전부 `box`로 채운다. 원통 화면이 생기기
전의 기록은 모두 사각이다. `언어` 열도 뒤에 끼웠다면 기존 줄은 `ko`로 채워도
좋다(그 화면은 한국어 페이지만 있었다).
...
// 붙일 시트: 이름이 'log'인 시트, 머리글 10칸

var SHEET_NAME = 'log';

// 받아 적을 언어. src/core/i18n/locales.ts의 LOCALES와 같아야 한다 — 여기
// 빠진 언어의 다운로드는 조용히 버려져 그 페이지 통계가 통째로 0이 된다.
// tests/track.test.ts가 이 목록과 LOCALES를 대조한다.
var LANGS = ['ko', 'en', 'zh-TW', 'zh-CN', 'ja'];
 ...
 * 치수 범위는 사각(100~400/50~300/40~200)과 원통(지름 80~300, 옆면 40~300,
 * 뚜껑 10~150)의 합집합이다 — 두 종류가 같은 칸을 쓰므로 여기서 종류별로
 * 따로 못 잰다.
 */
function valid(d) {
  return d
    ...
    && (d.kind === 'box' || d.kind === 'round')
    && LANGS.indexOf(d.lang) !== -1;
}
...
function tooMany() {
  ...
  if (!last || now - Number(last) > 60000) {
    PropertiesService.getScriptProperties().setProperty('last', String(now));
    return false;
  }
  return true;
}
...
- **한 종류의 인기**: 행에 `가로`·`높이`·`바닥폭`, 값에 `탭`을 **고유 개수**로
...
- **구간으로 보기**: 빈 열에 `=FLOOR(C2,50)` 같은 식을 붙여 50mm 단위로 묶은
  ...
  다시 볼 수 있다
- **언어별로 보기**: 필터에 `언어`를 더해 페이지별 방문 비율을 본다.
