...
- **네모네모 손잡이 파우치** — 가로·폭·옆면 높이·뚜껑 높이 → `/square-pouch/` ([계산](#네모네모-손잡이-파우치)) · *시험 중, silbap 회원 전용으로 넘길 예정*
...
`/round-pouch/`는 뚜껑이 열리는 원통 파우치를 만든다. 완성 치수는 셋이다 — 지름 `D`, 옆면 높이 `Hs`, 뚜껑 높이 `Hl`. 시접 `S`(10mm)와 지퍼 여유 `Z`(10mm)는 사각 파우치와 같은 상수를 쓴다.
...
**앞면 두 단에는 너치가 둘씩 있다.** 원과 박는 긴 변 — 윗단은 위 변, 아랫단은 아래 변 — 에, 양 끝 완성선에서 뒷면 길이 `Lb`만큼 들어온 자리다. 지퍼를 다는 변에는 없다. 두 너치는 늘 엇갈리지 않는다: `Lf − 2Lb = C(1 − 3r)`이고 `r ≤ 0.3`이라 `0.1C` 이상 남는다.

너치는 재단선과 같은 색·굵기로, 재단선에서 시접의 절반(5mm)만 들어간다. 완성선을 넘으면 박은 뒤에도 칼집이 겉에 남는다. 시접 없이 뽑으면 2mm로 줄인다 — 띠 조각의 식서선이 완성선에서 3mm 떨어져 있어 그보다 짧아야 닿지 않는다.

### 조각마다 무엇이 적히나

조각이 넷으로 흩어져 있어, 다 오려 놓고 나면 어느 도안의 조각인지 알 방법이 없었다. 그래서 조각마다 세 줄이 찍힌다.

| | 모든 조각 | 출처 덩어리가 앉는 조각 하나 |
|---|---|---|
| 조각 이름 (`앞면 윗단`…) | ○ | ○ |
| 파우치 이름 + 치수 (`동글동글 원통 파우치 130*130*30`) | ○ | ○ (크게) |
| 계정 (`@silsuni_lab`) | ○ | ○ (크게) |
| 권유 한 줄 | ✕ | ○ |

**치수는 파우치 이름과 같은 줄에 붙인다.** 줄을 따로 두면 완성 높이가 20mm뿐인 조각에 세 줄이 안 들어간다. 같은 줄에 이으면 세로 자리를 더 쓰지 않는다.

**폭이 모자라면 치수를 뗀다.** 뒷면은 둘레의 10%까지 좁아질 수 있어(지름 80이면 25mm) 치수를 붙인 줄이 조각 밖으로 나간다. 그때는 이름만 남기고, 이름조차 안 들어가면 그 줄을 통째로 건너뛴다. 치수는 출처 덩어리에 어차피 한 번 적히므로, 조각 밖으로 삐져나간 글자보다 짧은 이름이 낫다.

**자리는 조각 모양을 따른다.** 사각 조각은 이름을 완성선 바로 안쪽 맨 위에 두고 나머지 두 줄을 그 아래 남은 자리 한가운데에 앉힌다 — 이름을 가운데 두면 출처 덩어리와 겹친다. 원 조각은 셋을 한 덩어리로 묶어 원 한가운데에 쌓는다. 원은 출처 덩어리 후보에서 빠져 있어 겹칠 것이 없고, 셋을 나눠 놓으면 이름만 한가운데 뜨고 나머지가 아래로 처져 한 덩어리로 안 읽힌다. 가로는 어느 조각에서나 가운데 정렬이다.

원에서 덩어리를 가운데에 맞출 때는 기준선이 아니라 **글자가 실제로 차지하는 위아래 끝**을 기준으로 삼는다. 기준선만 가운데에 두면 아래로 뻗은 두 줄만큼 덩어리가 위로 치우친다.

**출처 덩어리는 그것을 담을 수 있는 조각에 앉힌다.** 예전에는 넓이가 가장 큰 사각 조각을 골랐는데, 넓이만 보면 담지도 못할 조각이 뽑힌다. 납작 파우치(100/50/20)의 앞면 두 단은 251×20으로 가장 넓지만 완성 높이가 20mm뿐이라, 조각 이름 몫을 빼면 9.4mm만 남는다. 덩어리는 23.6mm가 필요해 글자가 완성선을 넘고 재단선 위까지 나갔다. 지금은 세로·가로를 모두 재서 담을 수 있는 것 중 가장 넓은 조각을 고른다.

**하나도 못 담는 조합이 있다.** 그 납작 파우치에 뒷면 10%를 고르면 앞면 두 단은 너무 낮고(20mm) 뒷면은 너무 좁다(31mm). 그때는 가장 덜 모자란 조각을 고르고 덩어리 쪽에서 배율을 1 아래로 내려 맞춘다(`TITLE_SCALE_MIN`=0.5). 넓이로 고르면 세로가 9.4mm뿐인 조각이 뽑혀 훨씬 크게 넘친다. `tests/round-pdf.test.ts`가 프리셋 셋 × 뒷면 비율 다섯, 열다섯 조합을 모두 확인한다.

**배율은 세로만이 아니라 가로도 잡는다.** 세로가 넉넉하고 가로가 좁은 조각에서 배율이 상한(2.25)까지 올라가면 이번엔 옆으로 넘친다. 사각 파우치도 같아서 — 100×200×100이면 덩어리가 130mm인데 앞판은 120mm다 — 두 종류 모두 폭으로 한 번 더 조인다. 넓은 도안에서는 폭이 배율을 잡지 않으므로 예전 그대로 나온다.
...
`/square-pouch/`는 원통 파우치의 원을 직사각형으로 바꾼 뚜껑 파우치다. 지퍼가 앞과 양옆을 돌아 뒤 모서리까지 들어가고, 뒷면 가운데가 경첩이 되며, 뚜껑 위에 손잡이가 달린다.

> **silbap 회원 전용으로 넘길 도구다.** 만드는 동안만 여기 감춰 둔다 — `noindex`이고, 사각 화면·sitemap에서 링크하지 않고, 다운로드 기록을 남기지 않는다(`tests/pages.test.ts`가 지킨다). 한국어 한 벌뿐이다. silbap에 올라가면 `/round-pouch/`처럼 넘기는 안내로 바꾼다.

완성 치수는 넷이다 — 가로 `W`, 폭 `D`, 옆면 높이 `Hs`, 뚜껑 높이 `Hl`. 뒷면 비율 `r`은 원통과 같은 다섯 선택지다.
...
앞면 길이 Lf = P − Lb
몸통 높이 Hb = Hs − Hl − Z
손잡이    Lh = round(1.3 × W), 완성 폭 25 (반 접어 박으므로 펼친 폭 50)
```
...
**경첩은 뒷면 안에 있어야 한다(`Lb ≤ W`).** 넘으면 경첩이 옆 모서리를 돌아 뚜껑이 비틀려 열린다. 폭이 가로를 넘지 못하게 막아 두어(돌려 놓으면 같은 파우치다) 25%까지는 늘 되고, 잠길 수 있는 것은 30%뿐이다 — 폭이 가로의 2/3를 넘을 때. 화면은 그 선택지를 흐리게 두고, 고른 값이 잠기면 되는 것 중 가장 큰 값으로 내린다.

**너치는 모서리 자리다.** 앞면 두 단은 뚜껑·바닥과 박는 변에 넷 — 띠 끝에서 `a = (W − Lb)/2`, 그리고 `a+D`, `a+D+W`, `a+2D+W`. 뚜껑·바닥은 뒷변에 경첩 양 끝(`a`, `a+Lb`)과 좌우 변 한가운데(손잡이 끼울 자리)에 찍는다. 한 본으로 두 장을 뜨므로 바닥에서는 손잡이 너치를 무시한다. 경첩이 뒷면을 꽉 채우면(`a = 0`) 조각 끝과 겹치는 너치는 뺀다.

**손잡이에만 접힘선이 있다.** 사각 도안의 접힘선과 같은 연회색 점선이고, 화면에도 긋는다 — 사각 미리보기가 접힘선을 빼는 까닭(완성선과 한 직선을 나눠 가져 지그재그로 보인다)이 여기엔 없다. 가운데로 이 선이 지나가므로 출처 덩어리 후보에서 빠지고, 파우치 이름·계정 두 줄도 찍지 않는다.

**뚜껑·바닥 모서리를 둥글릴 수 있다** — 각지게(기본)·10·20·30mm 중에서 고른다. 반지름 `R`은 완성선 기준이고 폭의 1/4까지다(폭 80부터 20, 120부터 30). 둥글리면 세 가지가 따라 바뀐다.

- **둘레가 준다.** 모서리마다 직각 두 변(`2R`) 대신 1/4 호(`πR/2`)가 들어가 `P = 2(W + D) − (8 − 2π)R`. 앞면 띠와 뒷면이 이 둘레를 나눠 가져야 뚜껑과 맞는다.
- **너치는 호의 한가운데다.** 띠에서는 `a = (W − 2R − Lb)/2`부터 호 하나(`q = πR/2`)의 절반씩 들어간 네 자리, 뚜껑·바닥에서는 호의 45° 자리에 찍어 둘을 맞대면 호가 호에 얹힌다.
- **경첩은 뒷변의 곧은 부분 안에만 든다(`Lb ≤ W − 2R`).** 그래서 둥글리면 25%도 잠길 수 있다. 오류 문구도 가로가 아니라 곧은 부분 길이를 말한다.

재단선은 완성선의 호와 같은 중심에서 시접만큼 나간 곡선(반지름 `R + S`)이라 시접 폭이 모서리에서도 고르다. 조각 이름에 `R20`이 붙고, 파일 이름에도 `-r20`이 붙는다. 식서선은 반지름만큼 위로 올려 호 밖으로 나가지 않게 한다. 완성 예상 그림은 곡면이 있어 면을 셋으로 나눠 칠하지 못하므로, 윤곽을 촘촘히 찍어 투영하고 몸통을 위·아래 윤곽의 볼록 껍질로 칠한다.

| 값 | 범위 |
|---|---|
| `W` | 100 ~ 300 (손잡이에 손이 들어가는 최소) |
| `D` | 50 ~ 200, `D ≤ W` |
| `Hs`, `Hl` | 원통과 같다 |

나머지(배치 세 줄, 조각 표시, 출처 덩어리, 너치 길이)는 원통과 같다. 원통 파일은 silbap에 넘긴 원본이라 고치지 않고 `src/core/square/`·`src/ui/square/`에 한 벌을 두었다.
...
**페이지가 둘이다.** 나누면 각 화면이 자기 제목을 갖고, 종류를 고르는 UI도 필요 없다. `vite.config.ts`가 진입점 둘을 잡고, `base: './'`라 `/round-pouch/`에서 자산 경로가 `../assets/`로 알아서 맞는다.
