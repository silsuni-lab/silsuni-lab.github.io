...
조각이 넷으로 흩어져 있어, 다 오려 놓고 나면 어느 도안의 조각인지 알 방법이 없었다. 그래서 조각마다 세 가지가 찍힌다.
...
**치수는 조각마다 되풀이하지 않는다.** 조각마다 넣으면 종이가 빽빽해지고, 짧은 조각에는 들어가지도 않는다. 치수가 필요한 건 "이 종이가 어떤 도안이냐"를 물을 때고 그건 한 번이면 된다.
...
**출처 덩어리는 그것을 담을 수 있는 조각에 앉힌다.** 예전에는 넓이가 가장 큰 사각 조각을 골랐는데, 넓이만 보면 담지도 못할 조각이 뽑힌다. 납작 파우치(100/50/20)의 앞면 두 단은 251×20으로 가장 넓지만 완성 높이가 20mm뿐이라, 조각 이름 몫을 빼면 9.4mm만 남는다. 덩어리는 23.6mm가 필요해 글자가 완성선을 넘고 재단선 위까지 나갔다. 지금은 세로·가로를 모두 재서 담을 수 있는 것 중 가장 넓은 조각을 고른다 — 그 도안에서는 뒷면(63×50)이 뽑힌다. 뒷면은 완성 높이가 옆면 높이(최소 40)라 언제나 후보에 들어, 못 찾는 경우가 없다.
